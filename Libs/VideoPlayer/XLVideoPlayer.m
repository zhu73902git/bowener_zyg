...
@interface XLVideoPlayer ()<AVAssetResourceLoaderDelegate>
...
@property (nonatomic, assign) CGFloat totalDuration;    // 总缓存内容
@property (nonatomic, assign) CGFloat current;          // 当前缓存内容
...
/// 网络出现异常时 显示的图层视图
@property (nonatomic, strong) ZYGSelecteContinuePlayView    *showStateView;
@property (nonatomic, assign) BOOL  WWANPlay;   // 是否允许WWAN播放
...
- (void)createDevice{
    ...
    AVCaptureSession *cap =[[AVCaptureSession alloc] init];
    AVCaptureDevice *device =[AVCaptureDevice defaultDeviceWithMediaType:AVMediaTypeAudio];
    
    NSError *eror = nil;
    
    AVCaptureDeviceInput *av =[AVCaptureDeviceInput deviceInputWithDevice:device error:&eror];
    if (eror==nil) {
        if (av) {
            [cap addInput:av];
        }
...
- (void)testingCurrentNet{
    ...
    [[AFNetworkReachabilityManager sharedManager ] setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        switch (status) {
            case -1:
                BWLog(@"未知网络");
            [self chickNetIsWIFIOrOther];
                break;
            case 0:
                BWLog(@"网络不可用");
            [self chickNotNetWork];
...
#pragma mark  检测到无网络 
- (void)chickNotNetWork{
    self.activityIndicatorView.hidden = YES;
    [self addSubview:self.showStateView];
    [self.showStateView bringSubviewToFront:self];
    __weak typeof(self) weakSelf = self;
    self.showStateView.detailTitle = @"视频加载失败";
    self.showStateView.title = @"";
    self.showStateView.buttonTitle = @"点击重试";
    self.showStateView.touchBlock = ^(BOOL bol){
       
        [weakSelf finishChange];
        
    };
    [self.showStateView mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (void)chickNetIsWWAN{
    if (!_WWANPlay) {
        ...
        [self.showStateView bringSubviewToFront:self];
        __weak typeof(self) weakSelf = self;
        self.showStateView.selBlock = ^(BOOL bol){
            weakSelf.WWANPlay = YES;
            [weakSelf playOrPause:weakSelf.playOrPauseBtn];
        };
        [self.showStateView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.bottom.right.left.mas_equalTo(@0);
        }];
...
#pragma mark   检测到是 WI-FI 或其他网络
- (void)chickNetIsWIFIOrOther{
    if (self.showStateView) {
        [self.showStateView hideSelecteView];
        self.WWANPlay = NO;
    }
    if (self.playOrPauseBtn.isSelected) {
        [self.player play];
...
- (ZYGSelecteContinuePlayView *)showStateView{
    if (!_showStateView) {
        _showStateView =[[ZYGSelecteContinuePlayView alloc] init];
        _showStateView.backGroundColor =BWColor(1, 1, 1, 0.95);
    }
    return _showStateView;
}
...
#pragma mark   绑定tableView  以及视频的位置
...
-(void)addObserverToPlayerItem:(AVPlayerItem *)playerItem{
    ...
    //network loading progress   网络加载进度
    [playerItem addObserver:self forKeyPath:@"loadedTimeRanges" options:NSKeyValueObservingOptionNew context:nil];
    // playback Buffer Empty     播放缓冲区空
    [playerItem addObserver:self forKeyPath:@"playbackBufferEmpty" options:NSKeyValueObservingOptionNew context:nil];
    // playback Likely To KeepUp
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context{
    ...
    if ([keyPath isEqualToString:@"status"]) {
        ...
        BWLog(@"startSeconds %lf",startSeconds);
        BWLog(@"缓冲长度 - durationSeconds %lf",durationSeconds);
        BWLog(@"缓存总长度：%.2f",totalBuffer);
...
//            self.activityIndicatorView.center = self.center;
        ...
        BWLog(@"playback Buffer Empty  播放缓冲区空");
    
    }else if ([keyPath isEqualToString:@"playbackLikelyToKeepUp"]){
        
        BWLog(@"playback Likely To KeepUp");
    }
}
