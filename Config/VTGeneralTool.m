...
+ (NSString *)getChangeTime:(NSString *)duration{
    NSTimeInterval _interval=[duration doubleValue] / 1000.0;
...
+ (NSString *)translationNumIntoChinese:(NSInteger)integer{
    NSArray *arr = @[@"零",@"一",@"二",@"三",@"四",@"五",@"六",@"七",@"八",@"九",@"十"];
    if (integer<100) {
        if (integer<=10) {
            NSString *numS =[arr objectAtIndex:integer];
            NSString *numStr =[NSString stringWithFormat:@"%@",numS];
            return numStr;
        }else{
        
            if (integer%10==0) {    // 如果全为 10的整数倍
                NSString *numS =[arr objectAtIndex:integer/10];
                NSString *numStr =[NSString stringWithFormat:@"%@十",numS];
                return numStr;
            }else{      // 不全为 10的整数倍 有余数
                
                NSInteger shi = integer/10;     // 十位
                NSString *shiStr =[arr objectAtIndex:shi];
                NSInteger ge = integer%10;
                NSString *geStr = [arr objectAtIndex:ge];
                //
                
                
                if (integer<20) {
                    NSString *numStr =[NSString stringWithFormat:@"十%@",geStr];
                    return numStr;
                }else{
                    NSString *numStr =[NSString stringWithFormat:@"%@十%@",shiStr,geStr];
                    return numStr;
                }
            }
        ...
        if (integer%100==0) {
            NSString *numS =[arr objectAtIndex:integer/100];
            NSString *numStr =[NSString stringWithFormat:@"%@百",numS];
            return numStr;
        }else{
        
            NSInteger bai= integer/100;
            NSString *baiStr =[arr objectAtIndex:bai];    // 百位
            NSInteger shi = integer%100/10;     // 十位
            NSString *shiStr =[arr objectAtIndex:shi];
            NSInteger ge = integer%100%10;
            NSString *geStr = [arr objectAtIndex:ge];
            
            //
            NSString *numStr =[NSString stringWithFormat:@"%@百%@十%@",baiStr,shiStr,geStr];
            return numStr;
        }
...
// 将数值转换01格式
+ (NSString *)translationNumIntoSequeseNum:(NSInteger)integer{
    if (integer<10) {
        NSString *str =[NSString stringWithFormat:@"0%ld",integer];
        return str;
    }else{
...
+ (BOOL)isCheckPassWordCharaterString:(NSString *)str WithNum:(NSInteger)num{

    if (str.length>=num&&str.length<=20) {
        //6-20位数字和字母组成
